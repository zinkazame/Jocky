/*
 * reg_walk.h — JOCKY Phase 13.4: Registry Inspection Collector
...
/*
 * JOCKY_enum_persistence — walk HKLM + HKCU Run / RunOnce keys.
 * Returns total entry count, -1 on failure.
 */
int JOCKY_enum_persistence(const char *output_path);

/*
 * JOCKY_enum_services_reg — enumerate top-level service subkeys,
...
/*
 * JOCKY_enum_typed_urls — read HKCU TypedURLs (browser address-bar history).
 * Returns URL count, -1 on failure.
 */
int JOCKY_enum_typed_urls(const char *output_path);

/*
 * JOCKY_reg_report — combined wrapper: calls all three and writes a summary.
